...
export default function StockHistoryPage() {
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col text-gray-900">
            <div className="bg-white shadow-sm border-b shrink-0 transition-all duration-300">
                <div className="max-w-7xl mx-auto px-6 md:px-12 py-6"> {/* Increased padding */}
                    <div className="flex flex-col gap-6">
                        {/* Title Row */}
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900">Stock Price History</h1>
                            <p className="text-sm text-gray-600">Daily closing prices (Last 30 Days)</p>
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-6">
                            ...
                            <div className="flex-1">
                                <label className="block text-xs font-medium text-gray-500 mb-1">Sectors (Select Multiple)</label>
                                <div className="h-24 overflow-y-auto border rounded-md p-2 bg-gray-50 text-sm">
                                    {sectors.map(sector => (
                                        <div key={sector.id} className="flex items-center space-x-2 mb-1">
                                            ...
                                            />
                                            <label htmlFor={`sector-${sector.id}`} className="cursor-pointer select-none text-gray-700">
                                                {sector.name}
                                            </label>
                                        </div>
                                    ))}
                            ...
                            <div className="flex-1">
                                <label className="block text-xs font-medium text-gray-500 mb-1">Categories (Select Multiple)</label>
                                <div className="h-24 overflow-y-auto border rounded-md p-2 bg-gray-50 text-sm">
                                    {categories.map(cat => (
                                        <div key={cat.id} className="flex items-center space-x-2 mb-1">
                                            ...
                                            />
                                            <label htmlFor={`cat-${cat.id}`} className="cursor-pointer select-none text-gray-700">
                                                {cat.name}
                                            </label>
                                        </div>
                                    ))}
                        ...
                        <div className="flex flex-col md:flex-row gap-6 items-end">
                            {/* Stock Search */}
                            <div className="flex-1">
                                <label className="block text-xs font-medium text-gray-500 mb-1">Search Stock</label>
                                <input
                                    type="text"
                                    placeholder="Enter symbol..."
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    className="block w-full px-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                />
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="bg-white border rounded px-3 py-2 text-sm text-gray-900 outline-none focus:ring-2 focus:ring-blue-500"
                                    />
                                </div>
                            ...
                            <div className="overflow-auto flex-1">
                                <table className="min-w-full divide-y divide-gray-200 border-separate" style={{ borderSpacing: 0 }}>
                                    <thead className="bg-gray-50 sticky top-0 z-10">
                                        <tr>
                                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider sticky left-0 bg-gray-50 border-b border-r shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] z-20 w-32 min-w-[120px]">
                                                Stock
                                            </th>
                                            {pivotData.dates.map(date => (
                                                <th key={date} className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider border-b min-w-[100px]">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {pivotData.rows.map((row) => (
                                            <tr key={row.symbol} className="hover:bg-gray-50">
                                                <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900 sticky left-0 bg-white border-r shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] z-10">
                                                    {row.symbol}
                                                </td>
                                                {pivotData.dates.map(date => {
                                                    const price = row.prices[date];
                                                    return (
                                                        <td key={`${row.symbol}-${date}`} className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 text-right">
                                                            {price !== undefined ?
